...
class BudgetType extends BaseController
{
	...
	{
		if (!$this->validate(['name' => ['rules' => 'is_unique_ignore_delete[budget_type.name]']])) {
	...
	public function updateType()
	{
		...
		$validationRules = [
			'editName' => "required|is_unique_ignore_delete[budget_type.name,{$ID}]",
		
		];
		
		if (!$this->validate($validationRules)) {
			session()->setFlashdata('notif_error', '<b>Failed to update BudgetType Code already exit!</b> ' . implode(' ', $this->validator->getErrors()));
			return redirect()->to(base_url('BudgetType'));
		}

	    // เรียกใช้การตรวจสอบจาก model
		// if (!$this->BudgetTypeModel->validateUniqueName($editName, $ID)) {
		// 	session()->setFlashdata('notif_error', '<b>Failed to update. BudgetType already exists or is deleted!</b>');
		// 	return redirect()->to(base_url('BudgetType'));
		// }
